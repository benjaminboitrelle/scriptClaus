...
class TelSegment(object): 
  ...
  #
  def fitSegment(self, det, qbyp, bField, milleFile, consBL=None, fitDUTscat=False):
    ...
    for planeID in sorted(det.keys(), key=lambda p: det[p].getZintersect() - (overlap if p in self.__hits else 0.)):
      ...
      # describe DUT scattering instead by local parameters?
      if planeID > 5 and fitDUTscat:
        sDUT = sArc
        XbyX0 = 0.
