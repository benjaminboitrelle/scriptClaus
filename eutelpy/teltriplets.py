...
class TelDoublet(object):
  ...
  def match(self, hit, distCut):
    ...
    if dx < distCutX[0] or dx > distCutX[1]:
      return None
    #
    return (dx, dy)
...
class TelTriplets(object):    
  ...
  def findTracks(self, hits, qbyp=0., bfac=(0., 0.), zMag=0.):   
    ...
    #doubletCutX, doubletCutY, tripletCutX, tripletCutY, slopeCutX, slopeCutY, positionCutX, positionCutY = self.__cuts[:8]
    doubletCutX, doubletCutY, tripletCutX, tripletCutY, slopeCutX, slopeCutY, positionCutX, positionCutY, dutCutX, dutCutY = self.__cuts[:10]
    # curvature
    ...
    for lcenter in [1, 4]:
      for hit1 in self.__hits[lcenter - 1]:
        for hit3 in self.__hits[lcenter + 1]:
          ...
          if dy < doubletCutY[0] or dy > doubletCutY[1]: #PHILL
            continue
          if dx < doubletCutX[0] or dx > doubletCutX[1]: #PHILL
            continue
          if self.__hists is not None:
            self.__hists.addEntry("doubletDx", dx)
            self.__hists.addEntry("doubletDy", dy)
          # take doublet
          #if tripletCut is None:
          if tripletCutX or tripletCutY is None:
            self.__triplets[lcenter / 3].append(TelTriplet([hit1, hit3], aDoublet, None))
            continue
          # validate triplet    
          for hit2 in self.__hits[lcenter]:
            #trp = aDoublet.match(hit2, tripletCut)
            trp = aDoublet.match(hit2, tripletCutX, tripletCutY)
            if trp is not None:
              nTrp += 1
    ...
    for i1, trp1 in enumerate(self.__triplets[0]):
      ...
      for i2, trp2 in enumerate(self.__triplets[1]):
        ...
        #print " slopes ", slope1[0], slope1[1], slope2[0], slope2[1]
        #if abs(dslopeX) > slopeCut[0] or abs(dslopeY) > slopeCut[1]:
        if dslopeX < slopeCutX[0] or dslopeX > slopeCutX[1]:
          continue
        if dslopeY < slopeCutY[0] or dslopeY > slopeCutY[1]:
          continue
        if self.__hists is not None:
        ...
        if dposX > positionCutX[0] and dposX < positionCutX[1] and dposY > positionCutY[0] and dposY < positionCutY[1]:
  ...
  def matchDUT(self):
    ...
    #distCut = self.__cuts4]
    dutCutX = self.__cuts[8]
    dutCutY = self.__cuts[9]
    for l, hits in self.__hits.iteritems():
      ...
      for i, h in enumerate(hits):  
        ...
        for j, seg in enumerate(self.__seg):
          ...
          dx = x - px
          dy =y - py
          print " dut ", l, px, py, x, y
          #if dx < distCut[0] and dy < distCut[1]:
          #if dx < distCutX[0] or dx > distCutX[1]and dy < distCutY[0] or dy > distCutY[1]:
          if dx > dutCutX[0] and dx < dutCutX[1] and dy > dutCutY[0] and  dy < dutCutY[1]:
            matches.append((i, j))
            h.addMatch()
            seg.addMatch()
            if self.__hists is not None:
              if h.getPlane() == 128:
                self.__hists.addEntry("OKF4-dx", x - px)
                self.__hists.addEntry("OKF4-dy", y - py)
              elif h.getPlane() == 129:
                self.__hists.addEntry("OKF7-dx", x - px)
                self.__hists.addEntry("OKF7-dy", y - py)
              else:
                print "DUTs not defined"
